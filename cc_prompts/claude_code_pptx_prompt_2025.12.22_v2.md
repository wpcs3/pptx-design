...
# Poppler configuration (installed via conda-forge)
# The user has installed Poppler using: conda install -c conda-forge poppler
# in a conda environment named "pptx-design"
#
# Detection logic:
# 1. First check if running inside an activated conda environment
# 2. Fall back to explicit path if not activated
# 3. Fall back to None (system PATH) as last resort
def get_poppler_path() -> str | None:
    ...
    for fallback in fallback_paths:
        if (fallback / 'pdftoppm.exe').exists():
            return str(fallback)
...
**Important:** The user has installed Poppler via conda-forge in an environment called `pptx-design`. When running this project, ensure the environment is activated:
...
If Poppler is not found, verify the installation with:
```bash
where pdftoppm
```
...
**Using pdf2image with Poppler (from config):**
```python
from pdf2image import convert_from_path
from config import POPPLER_PATH, RENDER_DPI

def pdf_to_images(pdf_path: Path, output_dir: Path) -> list[Path]:
    """Convert PDF to PNG images using poppler via pdf2image."""
    images = convert_from_path(
        pdf_path,
        dpi=RENDER_DPI,
        poppler_path=POPPLER_PATH,  # Uses conda-forge installation
        fmt='png'
    )
...
- POPPLER_PATH is auto-detected from the conda environment; ensure `conda activate pptx-design` before running
...
**Note:** Some packages are installed via conda (in the `pptx-design` environment), others via pip.

**Conda packages (already installed in environment):**
```
# These are installed via: conda install -c conda-forge <package>
poppler
pdf2image
pillow
scikit-image
...
**Pip packages (install with pip after activating conda env):**
```
python-pptx>=0.6.21
...
**Combined requirements.txt for pip packages only:**
```
python-pptx>=0.6.21
anthropic>=0.18.0
click>=8.0.0
...
# Conda packages (if not already installed)
...
1. **Prerequisites**
   ...
   - LibreOffice installed (link to download: https://www.libreoffice.org/download/download-libreoffice/)
   - Miniforge/conda installed (link: https://conda-forge.org/download/)
...
2. **Environment Setup**
   ...
   # Install conda packages (includes Poppler)
   conda install -c conda-forge poppler pdf2image pillow scikit-image numpy
...
3. **Configuration**
   - LibreOffice path is auto-detected; override in `config.py` if needed
   - Poppler path is auto-detected from the conda environment
   - Adjusting iteration limits and thresholds in `config.py`
...
5. **How it works** (explain the iterative loop)

6. **Troubleshooting**
   - LibreOffice not found: Check `LIBREOFFICE_PATH` in `config.py`
   - Poppler not found: Ensure `conda activate pptx-design` was run
   - Verify Poppler: Run `where pdftoppm` in the activated environment
   - Font substitution issues: Install missing fonts system-wide
...
1. **Conda environment**: The user has set up a conda environment named `pptx-design` with Poppler installed via conda-forge. Before running any scripts, activate it:
   ```bash
   conda activate pptx-design
   ```

2. **LibreOffice installation**: The user must have LibreOffice installed. Check for it at startup and give clear error messages.

3. **Poppler via conda-forge**: Poppler is installed in the conda environment. The `config.py` includes auto-detection logic for the Poppler path. If `POPPLER_PATH` is `None`, pdf2image will look in the system PATH.

4. **Font availability**: Fonts must be installed on the system for LibreOffice to render them correctly. If a template uses a font not installed, LibreOffice will substitute.

5. **EMU conversions**: python-pptx uses English Metric Units internally. 914400 EMUs = 1 inch. The `Inches()`, `Pt()`, and `Cm()` helpers handle conversions.

6. **Iteration limits**: Set reasonable limits (10 iterations max) to prevent infinite loops if perfect matching isn't achievable.

7. **API costs**: If using Claude API for vision comparison, be aware of costs. Consider caching descriptions.

---

## Begin Implementation

**Before starting, verify the environment is set up:**
...
Start by creating the project structure and implementing `config.py`. Then move to `renderer.py` to verify LibreOffice and Poppler integration works on the user's Windows system.

Ask the user to confirm both LibreOffice and the conda environment are properly configured before proceeding with render tests.
